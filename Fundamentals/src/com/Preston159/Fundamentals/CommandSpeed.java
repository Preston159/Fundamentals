...
public class CommandSpeed {
	public static enum type { FLY, WALK }
	public static void run(CommandSender sender, Float speed, Player target) {
		if(target.isFlying())
			target.setFlySpeed(speed);
		else
			target.setWalkSpeed(speed);
	}
	public static void run(CommandSender sender, Float speed, Player target, type t) {
		if(t == type.WALK)
			target.setWalkSpeed(speed);
		if(t == type.FLY)
			target.setFlySpeed(speed);
