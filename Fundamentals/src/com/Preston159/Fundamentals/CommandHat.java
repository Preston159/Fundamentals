...
public class CommandHat {
	public static void run(Player p, ItemStack i, Boolean useItem) {
		if(!p.getInventory().contains(i)) {
			FundamentalsMessages.sendMessage("You don't have that item!", p);
			return;
		}
		Integer first = p.getInventory().first(i);
		ItemStack helmet = p.getInventory().getHelmet();
		p.getInventory().setHelmet(i);
		p.getInventory().setItem(first, new ItemStack(Material.AIR));
		if(helmet != null)
